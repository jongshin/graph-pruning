...
namespace CpT {
	...
	//inline for header only implementation
	inline RandomNumberEngine32 getRandomNumberEngine32Instance() {
		struct Seed {
			/* Linux guarantees that multiple threads get different random numbers when they read /dev/urandom concurrently.
	...
	class RandomPoissonGenerator : public RandomGenerator<unsigned int> {
		...
		public:
			explicit RandomPoissonGenerator(double mean)
			: engine(getRandomNumberEngine32Instance())
			, dist(mean) {
			}
	...
	class RandomPoissonGenerator64 : public RandomGenerator<unsigned long long> {
		private:
			typedef std::poisson_distribution<unsigned long long> Distribution;
		
		public:
			explicit RandomPoissonGenerator64(double mean)
			: engine(getRandomNumberEngine64Instance())
			, dist(mean) {
			}
			
			RandomPoissonGenerator64(const RandomPoissonGenerator64&) = default;
			
			RandomPoissonGenerator64(RandomPoissonGenerator64&&) = default;
			
			RandomPoissonGenerator64& operator=(const RandomPoissonGenerator64&) = default;
			
			RandomPoissonGenerator64& operator=(RandomPoissonGenerator64&&) = default;
	...
	class RandomUnitComplexGenerator : public RandomGenerator<std::complex<double>> {
		private:
			typedef std::uniform_real_distribution<> Distribution;
		
		public:
			/*
			 * range of the argument. lower bound is inclusive and upper bound is exclusive.
			 */
			RandomUnitComplexGenerator(double l, double u)
			: engine(getRandomNumberEngine32Instance())
			, dist(l, u){
			}
			
			RandomUnitComplexGenerator(const RandomUnitComplexGenerator&) = default;
			
			RandomUnitComplexGenerator(RandomUnitComplexGenerator&&) = default;
			
			RandomUnitComplexGenerator& operator=(const RandomUnitComplexGenerator&) = default;
			
			RandomUnitComplexGenerator& operator=(RandomUnitComplexGenerator&&) = default;
			
			ReturnType operator()() {
				return std::polar(1.0, dist(engine));
			}
